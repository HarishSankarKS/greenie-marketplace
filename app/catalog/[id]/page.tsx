...
export default function MaterialDetailPage({ params }: { params: { id: string } }) {
  ...
  const reviews = getReviewsForMaterial(params.id);
  const avgRatings = getAverageRatings(params.id);
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-white rounded-3xl border border-gray-100 h-72 md:h-96 flex items-center justify-center relative overflow-hidden shadow-sm">
              ...
              {isOutOfStock && (
                <div className="absolute inset-0 bg-gray-900/40 flex items-center justify-center">
                  <span className="text-white text-lg font-bold bg-gray-800/70 px-6 py-3 rounded-2xl">
                    Out of Stock
                  </span>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={label} className="bg-gray-50 rounded-xl p-3">
                    ...
                    <p className={`text-sm font-semibold ${label === "Available" && isOutOfStock ? "text-red-500" : "text-gray-900"}`}>
                      {value}
                    </p>
                  </div>
                ))}
            ...
            <div className="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm">
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {material.tags.map((t) => (
                  <span key={t} className="text-xs text-gray-500 bg-gray-100 px-3 py-1 rounded-full border border-gray-200">
                    #{t}
                  </span>
                ))}
              </div>
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                {avgRatings && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <StarRating value={Number(overallAvg)} />
                      <p className="text-xs text-gray-400 mt-0.5">{avgRatings.count} review{avgRatings.count !== 1 ? "s" : ""}</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="lg:col-span-2">
            <div className="sticky top-20 space-y-4">
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-2 mb-5 p-3 bg-green-50 rounded-xl border border-green-100">
                      ...
                      <p className="text-sm text-green-700 font-medium">
                        {material.availableQty.toLocaleString()} {material.unit}s in stock
                      </p>
                    </div>
                    ...
                    <div className="mb-5">
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => setQty(Math.max(material.minOrder, qty - material.minOrder))}
                          className="w-10 h-10 rounded-xl border border-gray-200 flex items-center justify-center text-gray-600 hover:border-green-400 hover:text-green-700 transition-colors font-bold text-lg"
                        >−</button>
                        <div className="flex-1 text-center">
                          <span className="text-lg font-bold text-gray-900">{qty}</span>
                          <span className="text-sm text-gray-400 ml-1">{material.unit}s</span>
                        </div>
                        <button
                          onClick={() => setQty(Math.min(material.availableQty, qty + material.minOrder))}
                          className="w-10 h-10 rounded-xl border border-gray-200 flex items-center justify-center text-gray-600 hover:border-green-400 hover:text-green-700 transition-colors font-bold text-lg"
                        >+</button>
                      </div>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl font-bold text-sm transition-all duration-200 shadow-md ${
                        addedToCart
                    ...
                    {alreadyInCart && !addedToCart && (
                      <p className="text-xs text-center text-green-600 mt-2">✓ Already in cart</p>
                    )}

                    <Link
                      href="/checkout"
                      className="block w-full text-center mt-2 py-3 border border-gray-200 rounded-2xl text-sm font-semibold text-gray-700 hover:border-green-400 hover:text-green-700 transition-colors"
                    >
                      Proceed to Checkout
                    </Link>
                  </>
                    ...
                    {notifySubmitted ? (
                      ...
                      <form onSubmit={handleNotifySubmit} className="space-y-3">
                        <label className="text-sm font-semibold text-gray-700 block">
                          <Bell className="w-4 h-4 inline mr-1.5 text-amber-500" />
                        ...
                        <input
                          ...
                          className="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-400/20 focus:border-amber-400"
                        />
                        <button
                          type="submit"
                          className="w-full py-3 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-2xl transition-colors text-sm"
                        >
                          Notify Me
                        </button>
                      </form>
              ...
              <div className="bg-green-50 rounded-2xl border border-green-100 p-4">
                {[
                  "✓ AI grade-verified at Transfer Station",
                  "✓ Real-time stock from Fleet Intelligence",
