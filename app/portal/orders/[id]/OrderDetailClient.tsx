...
import { MapPin, Truck, Check, Star, FileText } from "lucide-react";
      ...
      <div style={{ background: "var(--color-primary-900)", borderBottom: "3px solid var(--color-accent-amber)", padding: "28px 0" }}>
        <div className="container-page" style={{ maxWidth: 800 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, padding: "5px 14px", borderRadius: "var(--radius-full)", background: meta.bg, color: meta.color }}>{meta.label}</span>
          </div>
        </div>
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ background: "var(--color-white)", border: "1.5px solid var(--color-neutral-200)", borderRadius: "var(--radius-lg)", padding: "24px 28px", boxShadow: "var(--shadow-card)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
              ...
              <button onClick={() => generateInvoicePDF(order)} style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 16px", background: "var(--color-accent-amber)", border: "none", borderRadius: "var(--radius-md)", fontSize: 12, fontWeight: 700, cursor: "pointer", color: "var(--color-neutral-900)" }}>
          ...
          <div style={{ background: "var(--color-white)", border: "1.5px solid var(--color-neutral-200)", borderRadius: "var(--radius-lg)", padding: "24px 28px", boxShadow: "var(--shadow-card)" }}>
            ...
            <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid var(--color-neutral-200)", display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "var(--color-neutral-500)" }}><span>Subtotal</span><span>{formatCurrency(order.subtotal)}</span></div>
              {order.logisticsFee > 0 && <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "var(--color-neutral-500)" }}><span>Logistics</span><span>{formatCurrency(order.logisticsFee)}</span></div>}
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "var(--color-neutral-500)" }}><span>CGST (9%)</span><span>{formatCurrency(order.gst / 2)}</span></div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "var(--color-neutral-500)" }}><span>SGST (9%)</span><span>{formatCurrency(order.gst / 2)}</span></div>
          ...
          {order.deliveryMode === "delivery" && (
            <div style={{ background: "var(--color-white)", border: "1.5px solid var(--color-neutral-200)", borderRadius: "var(--radius-lg)", padding: "24px 28px", boxShadow: "var(--shadow-card)" }}>
              ...
              {isInTransit && (
                <Link href="/portal/tracker" style={{ marginTop: 16, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "12px", background: "#7C3AED", borderRadius: "var(--radius-md)", fontSize: 14, fontWeight: 700, color: "#fff", textDecoration: "none" }}>
          ...
          {isDelivered && (
            <div style={{ background: "var(--color-white)", border: "1.5px solid var(--color-neutral-200)", borderRadius: "var(--radius-lg)", padding: "24px 28px", boxShadow: "var(--shadow-card)" }}>
              ...
              {!confirmed ? (
                <button onClick={() => setConfirmed(true)} className="btn-secondary" style={{ width: "100%", justifyContent: "center", height: 46, display: "flex" }}>Confirm Receipt</button>
              ) : (
                <div style={{ padding: 20, background: "var(--color-primary-50)", border: "1.5px solid var(--color-primary-500)", borderRadius: "var(--radius-md)", textAlign: "center" }}>
                  ...
                  <Link href={`/portal/orders/${order.id}/review`} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, color: "var(--color-primary-700)", textDecoration: "none" }}>
